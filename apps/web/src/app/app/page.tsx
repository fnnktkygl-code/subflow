...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300 max-w-4xl mx-auto">
      ...
      {googleAccount && <GoogleDriveSyncCard variant="banner" />}
      ...
      <div className="rounded-japandi-2xl border p-5 sm:p-6 flex flex-col gap-4 bg-japandi-surface border-japandi-border shadow-japandi-sm">
        ...
        {calculateUpcomingOccurrences(subscriptions).length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {calculateUpcomingOccurrences(subscriptions).slice(0, 3).map(({ subscription: sub, dueDate }) => (
              <div
                key={sub.id}
                onClick={() => {
                  setEditingSub(sub);
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="text-[10px] text-japandi-muted">
                      {dueDate.toLocaleDateString(locale, { day: 'numeric', month: 'short' })}
                    </span>
                  </div>
                </div>
